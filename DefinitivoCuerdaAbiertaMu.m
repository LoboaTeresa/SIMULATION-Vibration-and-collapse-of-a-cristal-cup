...
at(1,:)=(SDcuerda(u,ntrozos)./dq.^2)-(mu*vt(1,:))+Golpecitos(1);
...
kiu1=ut(n-1,:);
kiv1=vt(n-1,:);
kia1=(SDcuerda(kiu1,ntrozos)./dq.^2)-(mu*kiv1)+Golpecitos(2);

kiu2=kiu1+kiv1*dt/2;
kiv2=kiv1+kia1*dt/2;
kia2=(SDcuerda(kiu2,ntrozos)./dq.^2)-(mu*kiv2);

kiu3=kiu1+kiv2*dt/2;
kiv3=kiv1+kia2*dt/2;
kia3=(SDcuerda(kiu3,ntrozos)./dq.^2)-(mu*kiv3);

kiu4=kiu1+kiu3*dt;
kiv4=kiv1+kiv3*dt;
kia4=(SDcuerda(kiu4,ntrozos)./dq.^2)-(mu*kiv4);


ut(n,:)=ut(n-1,:)+dt/6*(kiv1+2*kiv2+2*kiv3+kiv4);
vt(n,:)=vt(n-1,:)+dt/6*(kia1+2*kia2+2*kia3+kia4);
at(n,:)=(SDcuerda(ut(n,:),ntrozos)./dq.^2)-(mu*vt(n,:))+Golpecitos();
